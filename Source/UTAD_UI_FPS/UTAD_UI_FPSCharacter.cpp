...
void AUTAD_UI_FPSCharacter::BeginPlay()
{
	...
	if (m_oSkillTreeWidgetClass)
	{
		m_pSkillTreeWidget = CreateWidget(GetWorld(), m_oSkillTreeWidgetClass);
		m_pSkillTreeWidget->AddToViewport();
		m_pSkillTreeWidget->SetVisibility(ESlateVisibility::Collapsed);
	}
...
void AUTAD_UI_FPSCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		//Skill Tree
		EnhancedInputComponent->BindAction(m_pToggleSkillTree, ETriggerEvent::Started, this, &AUTAD_UI_FPSCharacter::ToggleSkillTree);
	}
}
...
void AUTAD_UI_FPSCharacter::ToggleSkillTree()
{
	if(m_oSkillTreeWidgetClass)
	{
		if(bCanToggledSkillTree)
		{
			m_pSkillTreeWidget->SetVisibility(ESlateVisibility::Visible);
			m_pPlayerController->SetInputMode(FInputModeGameAndUI());
			m_pPlayerController->SetShowMouseCursor(true);
			bCanToggledSkillTree = false;
		}
		else
		{
			m_pSkillTreeWidget->SetVisibility(ESlateVisibility::Collapsed);
			m_pPlayerController->SetInputMode(FInputModeGameOnly());
			m_pPlayerController->SetShowMouseCursor(false);
			bCanToggledSkillTree = true;
		}
	}
}
...
int16 AUTAD_UI_FPSCharacter::GetSkillPoints()
{
	return m_iSkillPoints;
}

void AUTAD_UI_FPSCharacter::SetSkillPoints(int16 _SkillPoints)
{
	m_iSkillPoints = _SkillPoints;
}
